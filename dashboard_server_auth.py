#!/usr/bin/env python3
"""
Dashboard MCP Server - Authentication Based
API integration with signin/registration/logout functionality
"""
...
logger = logging.getLogger("dashboard-mcp-server-auth")

# Dashboard configuration - will be set dynamically during signin
DASHBOARD_BASE_URL = os.getenv("DASHBOARD_BASE_URL")  # Default fallback
...
class AuthState:
    def __init__(self):
        ...
        self.dashboard_url: Optional[str] = None  # Store current dashboard URL

# Global auth state
auth_state = AuthState()

# Create FastMCP server with stateless HTTP for FastAPI mounting
mcp = FastMCP("dashboard-auth", stateless_http=True)
...
async def make_api_request(method: str, endpoint: str, data: Optional[Dict] = None, auth_required: bool = True) -> Dict:
    """Make HTTP request to dashboard API with dynamic authentication"""
    headers = {
        "Content-Type": "application/json",
        "User-Agent": "Dashboard-MCP-Server-Auth"
    }
    
    # Add authorization header if authenticated and required
    if auth_required:
        if not auth_state.is_authenticated or not auth_state.access_token:
            raise Exception("Authentication required. Please sign in first using signin() tool.")
        headers["Authorization"] = f"Bearer {auth_state.access_token}"
    
    # Use the current dashboard URL from auth state, or fallback to env variable
    base_url = auth_state.dashboard_url or DASHBOARD_BASE_URL
    if not base_url:
        raise Exception("No dashboard URL configured. Please use signin_to_dashboard() first.")
                    ...
                    try:
                        ...
                        async with session.request(method, url, headers=headers, json=data) as retry_response:
                            if retry_response.status >= 400:
                                ...
                                raise Exception(f"Dashboard API error {retry_response.status}: {error_text}")
...
async def refresh_access_token():
    ...
    data = {"refresh": auth_state.refresh_token}
    response = await make_api_request("POST", "/auth/token/refresh/", data=data, auth_required=False)
    auth_state.access_token = response.get("access")
    return response

# ===== AUTHENTICATION =====
@mcp.tool()
async def signin_to_dashboard(dashboard_url: str, username: str, password: str) -> str:
    """Sign in to a specific dashboard instance with username and password
    
    Args:
        dashboard_url: Full dashboard URL (e.g., 'https://client1.exitmvp.com/api/v1')
        username: Your username
        password: Your password
    """
    try:
        # Store the dashboard URL
        auth_state.dashboard_url = dashboard_url.rstrip('/')
        
        data = {
            "username": username,
            "password": password
        }
        response = await make_api_request("POST", "/auth/login/", data=data, auth_required=False)
        
        # Store authentication data
        auth_state.access_token = response.get("access")
        auth_state.refresh_token = response.get("refresh")
        auth_state.user_info = response.get("user", {})
        auth_state.is_authenticated = True
        
        return json.dumps({
            "success": True,
            "message": f"Successfully signed in to {dashboard_url}",
            "user": auth_state.user_info.get('username', 'User'),
            "dashboard_url": auth_state.dashboard_url
        }, indent=2)
    except Exception as e:
        auth_state.is_authenticated = False
        auth_state.dashboard_url = None
        return json.dumps({
            "success": False,
            "error": str(e),
            "message": "Sign in failed. Check your credentials and dashboard URL."
        }, indent=2)

@mcp.tool()
async def signin(username: str, password: str) -> str:
    """Sign in using the default dashboard URL from environment"""
    if not DASHBOARD_BASE_URL:
        return json.dumps({
            "success": False,
            "message": "No default dashboard URL configured. Use signin_to_dashboard() instead."
        }, indent=2)
    
    return await signin_to_dashboard(DASHBOARD_BASE_URL, username, password)

@mcp.tool()
async def register(username: str, email: str, password: str, first_name: str = "", last_name: str = "") -> str:
    ...
    try:
        ...
        response = await make_api_request("POST", "/auth/register/", data=data, auth_required=False)
        
        return json.dumps({
            "success": True,
            "message": f"Successfully registered user '{username}'. You can now sign in.",
            "user": response
        }, indent=2)
    except Exception as e:
        return json.dumps({
...
async def logout() -> str:
    ...
    try:
        if auth_state.refresh_token:
            # Try to invalidate the refresh token on server
            try:
                data = {"refresh": auth_state.refresh_token}
                await make_api_request("POST", "/auth/logout/", data=data, auth_required=False)
            except Exception:
                pass  # Continue with local logout even if server logout fails
        ...
        return json.dumps({
            ...
            "note": f"Server logout failed: {str(e)}"
        }, indent=2)
...
async def get_auth_status() -> str:
    ...
    return json.dumps({
        ...
        "user": auth_state.user_info if auth_state.is_authenticated else None,
        "has_access_token": bool(auth_state.access_token),
        "has_refresh_token": bool(auth_state.refresh_token)
    }, indent=2)

@mcp.tool()
async def test_connection() -> str:
    """Test if the API connection and authentication is working"""
    try:
        ...
        return json.dumps({
            ...
            "user": auth_state.user_info.get('username', 'Unknown'),
